...
def index():
    ...
    cur = conn.cursor()
    gene_data = "SELECT Loci FROM hpgdb_main"
    cur.execute(gene_data)
    gene_data_list = cur.fetchall()

    cur = conn.cursor()
    drug_data = "SELECT drugbank_Name FROM drugbank"
    cur.execute(drug_data)
    drug_data_list = cur.fetchall()

    cur = conn.cursor()
    target_data = "SELECT target_name FROM drugbank_target"
    cur.execute(target_data)
    target_data_list = cur.fetchall()

    cur = conn.cursor()
    sideeffects_data = "SELECT sideeffect_link FROM sider_effect"
    cur.execute(sideeffects_data)
    sideeffects_data_list = cur.fetchall()

    cur = conn.cursor()
    pathway_data = "SELECT pathway_name FROM pathway_table"
    cur.execute(pathway_data)
    pathway_data_list = cur.fetchall()

    cur = conn.cursor()
    disease_data = "SELECT kegg_disease_link FROM kegg_disease"
    cur.execute(disease_data)
    disease_data_list = cur.fetchall()

    cur.close()
    conn.close()
    context = {'drug': len(drug_data_list), 'target': len(target_data_list), 'side_effect': len(sideeffects_data_list), 'pathway': len(pathway_data_list), 'disease': len(disease_data_list), 'gene': len(gene_data_list)}
    return render_template('index.html',**context,a="https://apps.bdimg.com/libs/jquery/2.1.4/jquery.min.js",b="https://code.highcharts.com/highcharts.js")
...
def diseasePathway():
    ...
    return render_template('diseases.html',diseasePathwayInfo=diseasePathwayInfo)
...
def Gene():
    conn=getConnection()
    cur = conn.cursor()
    sql_hpgdb = "SELECT Loci FROM hpgdb_main"
    cur.execute(sql_hpgdb)
    hpgdb_list = cur.fetchall()
    #print("hpgdb_list",hpgdb_list)
    hpgdb_cur=conn.cursor()
    hpgdb_info_list=[]
    for id in hpgdb_list:
        if len(id)>0:
            id=list(id)[0]
            sql_hpgdb="SELECT Loci,Variants,Alleles,Phenotype,Link,Description,NUM FROM hpgdb_main WHERE Loci = '%s'" %id
            hpgdb_cur.execute(sql_hpgdb)
            item=hpgdb_cur.fetchone()
            hpgdb_info_list.append(item)
    hpgdb_cur.close()
    cur.close()
    conn.close()
    # print(hpgdb_info_list)
    return render_template('Gene.html', hpgdb_info_list=hpgdb_info_list)
    # print("hpgdb_info_list:",hpgdb_info_list)


@app.route('/gene_info', methods=['GET','POST'])
def Gene_info():
    num=request.args.get("num")
    # print(num)
    return render_template('%s.html' % num)
...
def search():
    ...
    if diseaseInfo3 != None:
        ...
        return redirect(url_for('searchForKeggDiseaseId', keggDiseaseId=keggDiseaseId))
    ### searchhpgdb 根据 Variants 查找hpgdb_main
    cur_hpgdb_Variants = conn.cursor()
    sql_hpgdb_Variants = 'select NUM from hpgdb_main where Variants LIKE "%s"' % args
    cur_hpgdb_Variants.execute(sql_hpgdb_Variants)
    hpgdbInfo1 = cur_hpgdb_Variants.fetchone()
    print(hpgdbInfo1)
    if hpgdbInfo1 != None:
        hpgdbId = hpgdbInfo1[0]
        print(hpgdbId)
        return render_template('%s.html' % hpgdbId)
    ### searchhpgdb 根据 Loci 查找hpgdb_main
    cur_hpgdb_loci = conn.cursor()
    sql_hpgdb_loci = 'select * from hpgdb_main where Loci LIKE "%s"' % args
    cur_hpgdb_loci.execute(sql_hpgdb_loci)
    hpgdbInfo2 = cur_hpgdb_loci.fetchone()
    hpgdb_info_list = []
    hpgdb_info_list.append(hpgdbInfo2)
    if hpgdbInfo2 != None:
        return render_template('gene_search.html', hpgdb_info_list=hpgdb_info_list)
    #cur_hpgdb_Phenotype = conn.cursor()
    #sql_hpgdb_Phenotype = 'select Phenotype from hpgdb_main where Loci LIKE "%s"' % args
    #cur_hpgdb_Phenotype.execute(sql_hpgdb_Phenotype)
    #hpgdbInfo3 = cur_hpgdb_Phenotype.fetchone()
    #if hpgdbInfo3 != None:
    #    hpgdbId3 = hpgdbInfo3[0]
    #    return render_template('%s.html' % hpgdbId3)
    else:
